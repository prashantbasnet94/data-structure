/*
        ...
        Kodane algo:
        Technique that is used to solve
                a. maximum subarray sum problem, which involves finding the contigous subarray with largest sum within an array of numbers
                b.  problems that require finding the maximum/minimum [sum || subarray]


                1. Max / Min subarray sum
                2. Contiguous Subarray Requirement:  if problem hints subarray must be contiguous
                3. Linear Time Complexity: Kodane big O => O(N), if the problem suggest linear time complexity requirement
                4. 1D array, if the problem involves processing a single array to find
                                a. Maximum/Minimum subarray sum
...
Some possible contiguous subarrays would be:
