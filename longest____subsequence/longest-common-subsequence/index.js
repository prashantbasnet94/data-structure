...
function lcsRevision(i, j , A, B, dp) {
    if (A[ i ] === undefined || B[ j ] === undefined) {
        return 0
    } else if (A[ i ] === B[ j ]) {
        return lcsRevision(i+1, j+1, A, B, dp)
    } else {
        return Math.max(lcsRevision(i +1, j, M, N), lcsRevision(i, j+ 1, M, N))
    }
}


function optimizingLCSRevision(i, j , A, B, dp) {
    if (A[ i ] === undefined || B[ j ] === undefined) {
        return 0
    } else if (A[ i ] === B[ j ]) {
        dp[ i + 1 ][ j + 1 ] = 1 + optimizingLCSRevision(i + 1, j + 1, A, B, dp)
        return dp[i+ 1][j+ 1]
    } else {
